...
As can be seen from this graph, the snake's dying lengths fit a normal distribution with mean $\mu = 23.15$ and standard deviation $\sigma = 5.99$. Note that because the dying length of a snake is a discrete variable being approximated by normal distribution, which is continuous, a continuity correction is needed. Since areas under a continuous distribution give probabilities rather the the height of the curve, for an integer value $x$ from a set of discrete values including $\{\ldots, x-1, x, x+1, \ldots\}$, $P(X=x)$ can be approximated to $P(x-0.5 < X < x+0.5)$. The halfway points are the boundaries of the bars that represent the area under the curve for the discrete value $x$.
...
\centering
	\begin{tikzpicture}
		...
		\addplot gnuplot [very thick,mark=none,color=green,domain=0:100] {350*erf(5.0*(20*x-463.0)/(599.0*sqrt(2))) + 0.4426*x*log(x) + 0.25*x^2 - 44.0429*x + 2850};
		\end{axis}
	\end{tikzpicture}
	\caption{Graphing $T(l_s)$}
